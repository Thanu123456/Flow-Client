...
const transformSettings = (s: any): PosSettings => ({
  ...
  blindCashCount: s.blind_cash_count ?? true,
  cashVarianceAlertThreshold: toNum(s.cash_variance_alert_threshold, 500),
...
const settingsPayload = (u: PosSettingsUpdate): Record<string, any> => {
  const map: Record<keyof PosSettingsUpdate, string> = {
    ...
    cashVarianceAlertThreshold: "cash_variance_alert_threshold",

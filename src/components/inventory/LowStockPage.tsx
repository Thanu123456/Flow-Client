...
import { Space, Tag, message, InputNumber } from "antd";
...
import { lowStockService } from "../../services/inventory/lowStockService";
...
const LowStockPage: React.FC = () => {
    ...
    }, [debouncedSearch, categoryId, threshold]);
    ...
    const columns = [
        ...
        },
        {
            title: "Status",
            key: "status",
            render: (record: StockListItem) => <Tag color="orange">{record.status}</Tag>,
        },
    ];
            ...
            actions={
                <Space>
                    <InputNumber
                        min={1}
                        value={threshold}
                        onChange={(val) => setThreshold(val ?? 20)}
